...
type UserStringLoader struct {
	...
	batch *userStringLoaderBatch

	// mutex to prevent races
	mu sync.Mutex
}

type userStringLoaderBatch struct {
	keys    []string
	data    []*model.User
	error   []error
	closing bool
...
func (l *UserStringLoader) LoadThunk(key string) func() (*model.User, error) {
	...
	if l.batch == nil {
		l.batch = &userStringLoaderBatch{done: make(chan struct{})}
	}
	batch := l.batch
...
func (l *UserStringLoader) unsafeSet(key string, value *model.User) {
	if l.cache == nil {
...
func (b *userStringLoaderBatch) keyIndex(l *UserStringLoader, key string) int {
...
func (b *userStringLoaderBatch) startTimer(l *UserStringLoader) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *userStringLoaderBatch) end(l *UserStringLoader) {
