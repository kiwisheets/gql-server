...
func (l *AddressLoader) LoadThunk(key int64) func() (*model.Address, error) {
	...
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*model.Address, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *AddressLoader) LoadAll(keys []int64) ([]*model.Address, []error) {
	results := make([]func() (*model.Address, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	addresss := make([]*model.Address, len(keys))
...
func (l *AddressLoader) unsafeSet(key int64, value *model.Address) {
	if l.cache == nil {
		l.cache = map[int64]*model.Address{}
	}
	l.cache[key] = value
}
...
func (b *addressLoaderBatch) keyIndex(l *AddressLoader, key int64) int {
